#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32u5x9j-dk/build-knsh-lvgl.sh
#
...
usage()
{
  ...
  printf 'Build STM32U5x9J-DK protected KNSh LVGL firmware for internal Flash.\n\n'
...
build_dir="../build"
image_prefix="${build_dir}/stm32u5x9j-dk-knsh-lvgl"
...
{
  mkdir -p "${build_dir}"
  find "${build_dir}" -maxdepth 1 -type f -name 'stm32u5x9j-dk-knsh-lvgl*' \
    -delete
}
...
printf '==> Cleaning STM32U5x9J-DK KNSh LVGL build outputs\n'
clean_build_dir
clean_lvgl_objects
...
printf '==> Building STM32U5x9J-DK protected KNSh LVGL for internal Flash\n'
make "-j${jobs}"
...
printf '\n==> Firmware outputs\n'
printf '  KNSh LVGL protected internal-Flash image:\n'
printf '    bin:        %s\n' "${image_prefix}.bin"
