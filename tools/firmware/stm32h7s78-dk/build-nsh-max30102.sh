#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32h7s78-dk/build-nsh-max30102.sh
#
...
usage()
{
  ...
  printf 'Build STM32H7S78-DK NXboot and NSH MAX30102 firmware.\n\n'
  ...
  printf '  build/stm32h7s78-dk-nsh-max30102.bin\n'
  printf '      Final NSH MAX30102 app image: [NXboot header][NuttX app raw binary].\n'
...
app_image_bin="${build_dir}/stm32h7s78-dk-nsh-max30102.bin"
...
printf '\n==> Building STM32H7S78-DK NSH MAX30102 NXboot app\n'
distclean_tree
configure_board stm32h7s78-dk:nsh-max30102
make "-j${jobs}"
...
printf '  NSH MAX30102 NXboot app image:\n'
printf '    file:       %s\n' "${app_image_bin}"
