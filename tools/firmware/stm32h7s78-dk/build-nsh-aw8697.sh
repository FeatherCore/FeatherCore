#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32h7s78-dk/build-nsh-aw8697.sh
#
...
usage()
{
  ...
  printf 'Build STM32H7S78-DK NXboot and NSH AW8697 haptic firmware.\n\n'
  ...
  printf '  build/stm32h7s78-dk-nsh-aw8697.bin\n'
  printf '      Final NSH AW8697 app image: [NXboot header][NuttX app raw binary].\n'
...
app_image_bin="${build_dir}/stm32h7s78-dk-nsh-aw8697.bin"
...
printf '\n==> Building STM32H7S78-DK NSH AW8697 NXboot app\n'
distclean_tree
configure_board stm32h7s78-dk:nsh-aw8697
make "-j${jobs}"
...
printf '  NSH AW8697 NXboot app image:\n'
printf '    file:       %s\n' "${app_image_bin}"
