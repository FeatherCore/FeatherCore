#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32h7s78-dk/build-nsh-bme280.sh
#
...
usage()
{
  ...
  printf 'Build STM32H7S78-DK NXboot and NSH BME280 firmware.\n\n'
  ...
  printf '  build/stm32h7s78-dk-nsh-bme280.bin\n'
  printf '      Final NSH BME280 app image: [NXboot header][NuttX app raw binary].\n'
...
app_image_bin="${build_dir}/stm32h7s78-dk-nsh-bme280.bin"
...
printf '\n==> Building STM32H7S78-DK NSH BME280 NXboot app\n'
distclean_tree
configure_board stm32h7s78-dk:nsh-bme280
make "-j${jobs}"
...
printf '  NSH BME280 NXboot app image:\n'
printf '    file:       %s\n' "${app_image_bin}"
