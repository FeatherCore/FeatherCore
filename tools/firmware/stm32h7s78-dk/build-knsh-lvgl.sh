#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32h7s78-dk/build-knsh-lvgl.sh
#
...
usage()
{
  ...
  printf 'Build STM32H7S78-DK NXboot and protected KNSh LVGL framebuffer firmware.\n\n'
  ...
  printf '  build/stm32h7s78-dk-knsh-lvgl.bin\n'
  printf '      Final protected KNSh LVGL app image: [NXboot header][kernel blob]\n'
...
app_image_bin="${build_dir}/stm32h7s78-dk-knsh-lvgl.bin"
...
verify_lvgl_config()
{
  ...
  do
    if ! grep -q "^${name}=y" .config; then
      printf 'ERROR: %s is not enabled by stm32h7s78-dk:knsh-lvgl\n' \
        "${name}" >&2
      missing=1
    fi
  done
...
printf '\n==> Building STM32H7S78-DK protected KNSh LVGL framebuffer app\n'
distclean_tree
configure_board stm32h7s78-dk:knsh-lvgl
verify_lvgl_config
prepare_lvgl_source
prepare_arm2d_source
clean_lvgl_objects
...
app_raw_bin="$(mktemp "${TMPDIR:-/tmp}/stm32h7s78-dk-knsh-lvgl-raw.XXXXXX")"
...
printf '    kernel:     %s bytes at 0x70000000 + %s\n' \
