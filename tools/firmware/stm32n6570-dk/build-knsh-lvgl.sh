#!/usr/bin/env bash
############################################################################
# tools/firmware/stm32n6570-dk/build-knsh-lvgl.sh
#
...
usage()
{
  ...
  printf 'Build STM32N6570-DK NXboot and protected KNSh LVGL framebuffer firmware.\n\n'
  printf 'Outputs are written to:\n\n'
  printf '  build/stm32n6570-dk-nxboot.bin\n'
  printf '      Program at XSPI2 NOR 0x70000000.\n\n'
  printf '  build/stm32n6570-dk-knsh-lvgl.bin\n'
  printf '      Program at XSPI2 NOR 0x70100000.\n\n'
  printf '  build/stm32n6570-dk-knsh-lvgl-full.bin\n'
...
app_image_bin="${build_dir}/stm32n6570-dk-knsh-lvgl.bin"
full_image_bin="${build_dir}/stm32n6570-dk-knsh-lvgl-full.bin"
...
verify_lvgl_config()
{
  ...
  # The LVGL protected build is also the main PSRAM stress image: LVGL stacks,
  # heap allocations, and framebuffer sync traffic all exercise XSPI1 PSRAM.
  # Keep these checks here so a future defconfig change cannot silently drop
  # the M55 PSRAM cache policy or the protected-syscall resume fixes.
  ...
  for name in \
    ...
    CONFIG_STM32N6_PSRAM_MPU_SHARE_NONE \
    CONFIG_STM32N6_PSRAM_MPU_WRITE_BACK \
    CONFIG_STM32N6_PSRAM_MPU_NO_WRITE_ALLOCATE \
  ...
  do
    if ! grep -q "^${name}=y" .config; then
      printf 'ERROR: %s is not enabled by stm32n6570-dk:knsh-lvgl\n' \
        "${name}" >&2
      missing=1
    fi
  done
  ...
  if [[ "$(config_value CONFIG_ARMV8M_SYSCALL_KERNEL_STACKSIZE)" != "8192" ]]; then
    printf 'ERROR: CONFIG_ARMV8M_SYSCALL_KERNEL_STACKSIZE must be 8192 for stm32n6570-dk:knsh-lvgl\n' >&2
    exit 1
  fi
}
...
if kernel_window <= 0:
    raise SystemExit("ERROR: CONFIG_NUTTX_USERSPACE must be above kernel")
...
printf '\n==> Building STM32N6570-DK protected KNSh LVGL framebuffer app\n'
distclean_tree
configure_board stm32n6570-dk:knsh-lvgl
verify_lvgl_config
prepare_lvgl_source
prepare_arm2d_source
clean_lvgl_objects
...
app_raw_bin="$(mktemp "${TMPDIR:-/tmp}/stm32n6570-dk-knsh-lvgl-raw.XXXXXX")"
...
printf '  KNSh LVGL protected app payload:\n'
printf '    size:       %s bytes\n' "$(file_size "${app_raw_bin}")"
printf '    kernel:     %s bytes at 0x70100000 + %s\n' \
...
printf '  KNSh LVGL NXboot app image:\n'
printf '    file:       %s\n' "${app_image_bin}"
...
printf '    user heap0:  internal SRAM bootstrap %s bytes\n' \
  "$(config_value CONFIG_STM32N6_PROTECTED_UHEAP_SIZE)"
printf '    user heap1:  PSRAM from offset %s\n' \
  "$(config_value CONFIG_STM32N6_PSRAM_HEAP_OFFSET)"
printf '    psram mpu:   SH_NONE + write-back + no-write-allocate\n'
printf '    syscall:     lazy-fpu, basepri-isb, kstack=%s, kstack-psp\n' \
