...
usage()
{
  ...
  printf 'Build NuttX simulator Wi-Fi hwsim STA2 image.\n\n'
  printf 'Outputs:\n'
  printf '  build/nuttx-sim-sta2      host executable for NuttX sim:hwsim_sta2\n'
  printf '  build/nuttx-sim-sta2.map  linker map, when produced by the build\n\n'
  printf 'Options:\n'
...
build_dir="../build"
sim_config="hwsim_sta2"
sim_exe="${build_dir}/nuttx-sim-sta2"
sim_map="${build_dir}/nuttx-sim-sta2.map"
...
{
  mkdir -p "${build_dir}"
  find "${build_dir}" -maxdepth 1 -type f -name 'nuttx-sim-sta2*' -delete
}
...
printf '==> Cleaning simulator STA2 build outputs\n'
clean_build_dir
...
printf '==> Building simulator STA2 image\n'
