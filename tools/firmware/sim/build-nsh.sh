#!/usr/bin/env bash
############################################################################
# tools/firmware/sim/build-nsh.sh
#
...
usage()
{
  ...
  printf 'Build NuttX simulator NSH.\n\n'
  printf 'Outputs:\n'
  printf '  build/sim-nsh      host executable for NuttX sim:nsh\n'
  printf '  build/sim-nsh.map  linker map, when produced by the build\n\n'
  printf 'Options:\n'
...
build_dir="../build"
sim_exe="${build_dir}/sim-nsh"
sim_map="${build_dir}/sim-nsh.map"
...
{
  mkdir -p "${build_dir}"
  find "${build_dir}" -maxdepth 1 -type f -name 'sim-nsh*' -delete
}
...
printf '==> Cleaning simulator build outputs\n'
clean_build_dir

if [[ "${clean_first}" -ne 0 ]]; then
  printf '==> Configuring sim:nsh\n'
  distclean_tree
  configure_board sim:nsh
else
