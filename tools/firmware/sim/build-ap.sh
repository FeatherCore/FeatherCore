...
usage()
{
  ...
  printf 'Build NuttX simulator Wi-Fi hwsim AP image.\n\n'
  printf 'Outputs:\n'
  printf '  build/nuttx-sim-ap      host executable for NuttX sim:hwsim_ap\n'
  printf '  build/nuttx-sim-ap.map  linker map, when produced by the build\n\n'
  printf 'Options:\n'
...
build_dir="../build"
sim_config="hwsim_ap"
sim_exe="${build_dir}/nuttx-sim-ap"
sim_map="${build_dir}/nuttx-sim-ap.map"
...
{
  mkdir -p "${build_dir}"
  find "${build_dir}" -maxdepth 1 -type f -name 'nuttx-sim-ap*' -delete
}
...
printf '==> Cleaning simulator AP build outputs\n'
clean_build_dir
...
printf '==> Building simulator AP image\n'
