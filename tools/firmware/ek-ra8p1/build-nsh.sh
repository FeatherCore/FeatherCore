...
usage()
{
  ...
  printf 'Build EK-RA8P1 NXboot and NSH NXboot app firmware.\n\n'
  printf 'Outputs:\n'
  printf '  build/ek-ra8p1-nxboot.bin  raw NuttX NXboot, program at MRAM 0x02000000\n'
  printf '  build/ek-ra8p1-nsh.bin     [NXboot header][NuttX app], program at OSPI0 CS1 0x90000000\n\n'
  printf 'Options:\n'
...
app_image_bin="${build_dir}/ek-ra8p1-nsh.bin"
...
printf '\n==> Building EK-RA8P1 NSH NXboot app\n'
distclean_tree
configure_board ek-ra8p1:nsh
make "-j${jobs}"
...
printf '    app vector: 0x90000000 + %s, normally 0x90000400\n\n' "${header_size}"
